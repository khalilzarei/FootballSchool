...
import com.khz.footballschool.data.dto.request.AttachPlayerToGuardianRequest
...
private val RELATIONS = listOf(
    "father" to "پدر", "mother" to "مادر",
    "grandfather" to "پدربزرگ", "grandmother" to "مادربزرگ",
    "uncle" to "عمو/دایی", "aunt" to "عمه/خاله", "other" to "سایر"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AttachPlayerToGuardianScreen(
    guardianId: Int,
    onBack: () -> Unit,
    onAttached: () -> Unit
) {
    ...
    LaunchedEffect(Unit) {
        when (val r = playerRepo.getPlayers(perPage = 100)) {
    ...
    Scaffold(
        ...
        topBar = { GlassTopBar(title = "افزودن بازیکن به سرپرست", onBack = onBack) }
    ) { padding ->
        if (loading) {
            ...
            Column(
                ...
                GlassButton(
                    ...
                    onClick = {
                        ...
                        error = null
                        scope.launch {
                            val r = guardianRepo.attachPlayer(
                                guardianId,
                                AttachPlayerToGuardianRequest(
                                    playerId = p.id,
...
private fun PlayerSelectableItem(player: Player, isSelected: Boolean, onClick: () -> Unit) {
    GlassCard3D(modifier = Modifier.fillMaxWidth().clickable(onClick = onClick),) {
        Row(

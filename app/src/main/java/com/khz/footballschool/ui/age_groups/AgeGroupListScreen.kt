...
// نمایش شمسی با ارقام فارسی — ورودی/خروجی داده همیشه میلادی لاتین است
private fun jalali(date: String?): String =
    date?.let { DateUtils.toPersianDigits(DateUtils.gregorianToJalali(it)) } ?: "-"
            ...
            when (val s = state) {
                ...
                is ListState.Error -> GlassCard3D(Modifier.fillMaxWidth()) {
                    Column(
                        ...
                        GlassButton("تلاش مجدد", onClick = { vm.refresh() })
                    }
                }
                        ...
                        if (groups.isEmpty()) {
                            GlassCard3D(Modifier.fillMaxWidth()) {
                                Column(
                                    ...
                                    Text("گروه سنی‌ای ثبت نشده است", color = Color.White.copy(0.6f))
                                    Text(
                                ...
                                items(groups, key = { it.id }) { g ->
                                    AgeGroupCard(
                                        ...
                                        onToggle = {
                                            scope.launch {
                                                busy = true
                                                when (val r = repo.toggleStatus(g.id, !g.isActive)) {
    ...
    GlassCard3D(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Row(
                ...
                TextButton(onClick = onShowPlayers, enabled = !busy) {
                    Text("بازیکنان", color = GoldPrimary)
                }
                TextButton(onClick = onEdit, enabled = !busy) {
                    Text("ویرایش", color = Color.White.copy(0.8f))
                }
...
/**
 * دیالوگ ایجاد/ویرایش گروه سنی: نام + بازه تاریخ تولد
 */
@Composable
private fun AgeGroupDialog(
    ...
    var fromDate by remember { mutableStateOf(initial?.birthDateFrom ?: "") }
    var toDate by remember { mutableStateOf(initial?.birthDateTo ?: "") }
    var sortOrderText by remember { mutableStateOf((initial?.sortOrder ?: 0).toString()) }
    var localError by remember { mutableStateOf<String?>(null) }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(if (isEdit) "ویرایش گروه سنی" else "گروه سنی جدید") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                // ─── بازه تاریخ تولد: انتخاب با تقویم شمسی، ارسال میلادی به سرور ───
                JalaliDateField(
                    label = "از تاریخ تولد",
                    gregorianValue = fromDate.takeIf { it.isNotBlank() },
                    onDatePicked = { fromDate = it }
                )
                JalaliDateField(
                    label = "تا تاریخ تولد",
                    gregorianValue = toDate.takeIf { it.isNotBlank() },
                    onDatePicked = { toDate = it }
                )
                GlassTextField(
                    value = sortOrderText,
...
/**
 * دیالوگ بازیکنان عضو گروه سنی (بر اساس بازه تاریخ تولد)
 */
@Composable
private fun AgeGroupPlayersDialog(
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("بازیکنان «${group.title}»") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                when {
                    players == null && loadError == null -> Row(
                        ...
                        Text("در حال بارگذاری...", color = Color.White.copy(0.6f))
                    }
                    ...
                    players.orEmpty().isEmpty() -> Text(
                        ...
                        items(players.orEmpty(), key = { it.id }) { p ->
                            Row(
                                Modifier.fillMaxWidth(),
                                verticalAlignment = Alignment.CenterVertically
                            ) {

...
object ApiErrorHandler {
    ...
    fun extractMessage(throwable: Throwable): String {
        return when (throwable) {
            is HttpException -> {
                val errorBody = throwable.response()?.errorBody() ?: return defaultHttpMessage(throwable.code())
                parseErrorBody(errorBody) ?: defaultHttpMessage(throwable.code())
            }
            is IOException -> "خطا در اتصال به سرور. لطفاً اتصال اینترنت خود را بررسی کنید."
            is JsonSyntaxException -> "خطا در پردازش پاسخ سرور"
            else -> throwable.message ?: "خطای ناشناخته"
        }
    }

...
// دوره پرداخت: ماهیانه یا فصلی (۳ ماهه)
private val BILLING_CYCLES = listOf(
    "monthly" to "ماهیانه (۱ ماه)",
    "seasonal" to "فصلی (۳ ماهه)"
)

/** جداکننده‌ی سه‌رقمی برای نمایش مبالغ — ورودی فقط رقم (10,000,000) */
private fun formatThousands(digits: String): String = if (digits.length <= 3) digits
else digits.reversed()
    .chunked(3)
    .joinToString(",")
    .reversed()

// روزهای هفته - قرارداد سرور 1=شنبه ... 7=جمعه
...
fun ClassFormScreen(
    ...
    var selectedAgeGroupIds by remember { mutableStateOf<Set<Int>>(emptySet()) }
    var showAgeGroupPicker by remember { mutableStateOf(false) }
    ...
    var seasonalFee by remember { mutableStateOf("") }
    ...
    LaunchedEffect(cls) {
        cls?.let {
            ...
            capacity = it.capacity?.toString()
                    ?: ""
            selectedAgeGroupIds = it.ageGroupIds.toSet()
            coachId = it.coachId
            assistantCoachId = it.assistantCoachId
            pricingType = it.pricingType?.takeIf { pt -> PRICING_TYPES.any { o -> o.first == pt } }
                    ?: "monthly"
            billingCycle = it.billingCycle?.let { bc -> if (bc == "quarterly") "seasonal" else bc }
                ?.takeIf { bc -> BILLING_CYCLES.any { o -> o.first == bc } }
                    ?: "monthly"
            monthlyFee = it.monthlyFee?.takeIf { f -> f > 0 }
                ?.let { f -> formatThousands(f.toString()) }
                    ?: ""
            seasonalFee = it.seasonalFee?.takeIf { f -> f > 0 }
                ?.let { f -> formatThousands(f.toString()) }
                    ?: ""
            sessionFee = it.sessionFee?.takeIf { f -> f > 0 }
                ?.let { f -> formatThousands(f.toString()) }
                    ?: ""
            registrationFee = it.registrationFee?.takeIf { f -> f > 0 }
                ?.let { f -> formatThousands(f.toString()) }
                    ?: ""
            ...
            Column(
                ...
                GlassCard3D {
                    Column(
                        ...
                        if (refsError == null) {
                            // گروه‌های سنی — چند انتخاب (عضویت = بازه تاریخ تولد)
                            val selectedGroupTitles = ageGroups.filter { it.id in selectedAgeGroupIds }
                                .joinToString("، ") { it.title }
                            GlassButton(
                                text = if (selectedGroupTitles.isEmpty()) {
                                    "گروه‌های سنی (اختیاری) — انتخاب کنید"
                                } else {
                                    "گروه‌های سنی: $selectedGroupTitles"
                                },
                                onClick = { showAgeGroupPicker = true },
                                modifier = Modifier.fillMaxWidth()
                            )
                ...
                // ═════════════════════════════════════════
                // قیمت‌گذاری - با پشتیبانی ماهیانه / فصلی
                // ═════════════════════════════════════════
                GlassCard3D {
                    Column(
                        modifier = Modifier.padding(16.dp),
                        ...
                        // دوره پرداخت - فقط وقتی ماهانه یا both است
                        ...
                        // شهریه بر اساس دوره
                        if (pricingType == "monthly" || pricingType == "both") {
                            if (billingCycle == "seasonal") {
                                GlassTextField3D(
                                    value = seasonalFee,
                                    onValueChange = { v -> seasonalFee = formatThousands(v.filter { it.isDigit() }) },
                                    label = "شهریه فصلی - ۳ ماهه (تومان)",
                                    keyboardType = KeyboardType.Number,
                                    leadingIcon = {
                                        Icon(
                                            Icons.Default.Money,
                                ...
                                // نمایش معادل ماهیانه برای راهنمایی
                                val sFee = seasonalFee.replace(
                                    ",",
                                    ""
                                )
                                    .toLongOrNull()
                                if (sFee != null && sFee > 0) {
                                    Text(
                                        "معادل ماهیانه: ${formatThousands((sFee / 3).toString())} تومان",
                                        style = MaterialTheme.typography.bodySmall,
                                        color = Color.White.copy(0.6f)
                                    )
                                }
                                // اگر کاربر ماهیانه هم وارد کرده بود، نمایش بده
                                if (monthlyFee.isNotBlank()) {
                                    Text(
                                        "شهریه ماهیانه قبلی: $monthlyFee تومان (برای محاسبه خودکار فصلی استفاده می‌شود اگر فصلی خالی باشد)",
                                ...
                                // اگر فصلی هم وارد شده، نمایش معادل
                                val mFee = monthlyFee.replace(
                                    ",",
                                    ""
                                )
                                    .toLongOrNull()
                                if (mFee != null && mFee > 0) {
                                    Text(
                                        "معادل فصلی (۳ ماهه): ${formatThousands((mFee * 3).toString())} تومان",
                                        style = MaterialTheme.typography.bodySmall,
                                        color = Color.White.copy(0.6f)
                                    )
                                }
                ...
                GlassButton(
                    ...
                    onClick = {
                        ...
                        val mFee = monthlyFee.replace(
                            ...
                            .toLongOrNull()
                        val seasFee = seasonalFee.replace(
                            ",",
                            ""
                        )
                            .toLongOrNull()
                        val sFee = sessionFee.replace(
                            ",",
                            ""
                        )
                            .toLongOrNull()
                        val rFee = registrationFee.replace(
                            ",",
                            ""
                        )
                            .toLongOrNull()
                        ...
                        formError = when {
                            title.trim().length < 3                                                                                                                                 -> "عنوان کلاس باید حداقل ۳ کاراکتر باشد"
                            cap != null && cap < 1                                                                                                                                  -> "ظرفیت باید حداقل ۱ باشد"
                            (pricingType == "monthly" || pricingType == "both") && billingCycle == "monthly" && (mFee == null || mFee <= 0L)                                        -> "شهریه ماهانه را وارد کنید (عددی بزرگ‌تر از صفر)"
                            (pricingType == "monthly" || pricingType == "both") && billingCycle == "seasonal" && (seasFee == null || seasFee <= 0L) && (mFee == null || mFee <= 0L) -> "شهریه فصلی را وارد کنید (یا حداقل شهریه ماهانه برای محاسبه خودکار)"
                            (pricingType == "session" || pricingType == "both") && (sFee == null || sFee <= 0L)                                                                     -> "هزینه هر جلسه را وارد کنید (عددی بزرگ‌تر از صفر)"
                            (mFee != null && mFee < 0) || (seasFee != null && seasFee < 0) || (sFee != null && sFee < 0) || (rFee != null && rFee < 0)                              -> "مبالغ نمی‌توانند منفی باشند"
                            coachId != null && assistantCoachId != null && coachId == assistantCoachId                                                                              -> "مربی اصلی و کمکی نمی‌توانند یکسان باشند"
                            startDateGregorian != null && endDateGregorian != null && endDateGregorian!! < startDateGregorian!!                                                     -> "تاریخ پایان نمی‌تواند قبل از تاریخ شروع باشد"
                            draftSchedules.any { it.weekday == 0 }                                                                                                                  -> "روز هفته را برای همه‌ی ردیف‌های برنامه هفتگی انتخاب کنید"
                            draftSchedules.any { !Regex("^([01]?\\d|2[0-3]):[0-5]\\d$").matches(it.start.trim()) || !Regex("^([01]?\\d|2[0-3]):[0-5]\\d$").matches(it.end.trim()) } -> "ساعت‌های برنامه هفتگی باید با فرمت HH:mm باشند (مثلاً 17:00)"
                            draftSchedules.any { it.start.trim() >= it.end.trim() }                                                                                                 -> "ساعت شروع باید قبل از ساعت پایان باشد"
                            draftSchedules.groupBy { it.weekday }
                                .any { (w, rows) -> w != 0 && rows.size > 1 }                                                                                                       -> "هر روز هفته فقط یک بار قابل انتخاب است"
                        ...
                        if (formError == null) {
                                        ...
                                        if (okDays > 0 && autoGenerate) {
                                            if (endDateGregorian != null) {
                                                when (val g = sessionRepo.generateSessionsForClass(savedClassId)) {
                                                    is NetworkResult.Success -> notes += if (g.data > 0) "${g.data} جلسه تولید شد"
                                                    else "جلسه جدیدی تولید نشد (تکراری یا خارج از بازه)"

                                                    is NetworkResult.Error   -> notes += "خطا در تولید جلسات: ${g.message}"
                                                    else                     -> {}
                                                }
                                            } else {
                            ...
                            if (classId != null) {
                                vm.update(
                                    id = classId,
                                    title = title.trim(),
                                    ageGroupIds = selectedAgeGroupIds.toList(),
                                    coachId = coachId,
                                    ...
                                    billingCycle = if (pricingType == "monthly" || pricingType == "both") billingCycle else null,
                                    monthlyFee = mFee,
                                    seasonalFee = seasFee,
                                    sessionFee = sFee,
    ...
    // ─── انتخاب چندگانه‌ی گروه‌های سنی ───
    if (showAgeGroupPicker) {
        androidx.compose.material3.AlertDialog(
            onDismissRequest = { showAgeGroupPicker = false },
            title = { Text("گروه‌های سنی کلاس") },
            text = {
                Column(
                    ...
                    if (ageGroups.isEmpty()) {
                        Text(
                            "گروه سنی تعریف نشده است؛ ابتدا از بخش «گروه‌بندی سنی» اضافه کنید.",
                            style = MaterialTheme.typography.bodyMedium,
                            color = Color.White
                        )
                    }
                    ageGroups.forEach { ag ->
                        Row(
                            ...
                            Checkbox(
                                checked = ag.id in selectedAgeGroupIds,
                                onCheckedChange = { checked ->
                                    selectedAgeGroupIds = if (checked) {
                                        selectedAgeGroupIds + ag.id
                                    } else {
                                        selectedAgeGroupIds - ag.id
                                    }
                                },
            ...
            confirmButton = {
                androidx.compose.material3.TextButton(onClick = { showAgeGroupPicker = false }) {
                    Text(
                        "انجام شد",
                        color = GoldPrimary
                    )

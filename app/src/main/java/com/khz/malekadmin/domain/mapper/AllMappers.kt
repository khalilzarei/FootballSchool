package com.khz.malekadmin.domain.mapper
...
fun ClassDto.toDomain(): FootballClass = FootballClass(
    ...
    ageGroupIds = ageGroupIds?.takeIf { it.isNotEmpty() }
            ?: ageGroupId?.let { listOf(it) }
            ?: emptyList(),
    ageGroups = ageGroups?.map { it.toDomain() }
            ?: ageGroup?.let { listOf(it.toDomain()) }
            ?: emptyList(),
    coachId = coachId,
...
fun InvoiceDto.toDomain(): Invoice {
    val sub = subtotal
            ?: totalAmount
            ?: 0L
    val disc = discountTotal
            ?: 0L
    val paid = paidTotal
            ?: paidAmount
            ?: 0L
    val remain = remainingTotal
            ?: remainingAmount
            ?: (sub - disc - paid).coerceAtLeast(0L)
    ...
    return Invoice(
        ...
        classDebts = classDebts?.map { it.toDomain() }
                ?: run {
                    // اگر بک‌اند class_debts نفرستاد، از items محاسبه کن (fallback)
                    val map = mutableMapOf<String, ClassDebt>()
                    items.orEmpty()
                        .forEach { dto ->
                            val key = dto.classId?.toString()
                                    ?: "other"
                            ...
                            if (existing == null) {
                                map[key] = ClassDebt(
                                    ...
                                    classTitle = dto.classTitle
                                            ?: if (dto.classId == null) "سایر هزینه‌ها" else "کلاس #${dto.classId}",
                                    ageGroupTitle = dto.ageGroupTitle,
                                    total = dto.total,
                                    itemsCount = 1
                                )
                            } else {
                                map[key] = existing.copy(
                                    total = existing.total + dto.total,
...
fun InvoiceClassDebtDto.toDomain(): ClassDebt = ClassDebt(
    classId = classId,
    classTitle = classTitle
            ?: if (classId == null) "سایر هزینه‌ها" else "کلاس #$classId",
    ageGroupTitle = ageGroupTitle,
    total = total
            ?: 0L,
    paid = paid
...
fun MatchPlayerDto.toDomain(): MatchPlayer {
    ...
    val resolvedPlayer = player?.toDomain()
            ?: run {
                ...
                // فقط اگر حداقل یک نام داریم، Player موقت بساز
                if (full != null) {
                    Player(
                        id = playerId,
                        userId = null,

...
import com.khz.malekadmin.data.dto.request.*
import com.khz.malekadmin.data.dto.response.MatchDto
import com.khz.malekadmin.data.dto.response.MatchPlayerDto
import com.khz.malekadmin.data.remote.MatchApi
import com.khz.malekadmin.domain.mapper.toDomain
import com.khz.malekadmin.domain.model.Match
import com.khz.malekadmin.domain.model.MatchPlayer
...
class MatchRepository(private val api: MatchApi) {
    ...
    suspend fun getMatches(
        ...
        classId: Int? = null,
        ageGroupId: Int? = null
    ): NetworkResult<List<Match>> = try {
        val r = api.getMatches(
            page,
            perPage,
            q,
            status,
    ...
    suspend fun getMatchDetail(id: Int): NetworkResult<Match> = getMatch(id)

    suspend fun updateMatch(
        id: Int,
        request: UpdateMatchRequest
    ): NetworkResult<Match> = try {
        val r = api.updateMatch(
            id,
            request
        )
        val dto = r.data.unwrap<MatchDto>("match")
    ...
    suspend fun addPlayer(
        id: Int,
        request: AddMatchPlayerRequest
    ): NetworkResult<MatchPlayer> = try {
        val r = api.addPlayer(
            id,
            request
        )
    ...
    suspend fun updatePlayer(
        id: Int,
        request: UpdateMatchPlayerRequest
    ): NetworkResult<MatchPlayer> = try {
        val r = api.updatePlayer(
            id,
            request
        )
        val dto = r.data.unwrap<MatchPlayerDto>("match_player")
        if (r.success && dto != null) NetworkResult.Success(dto.toDomain())
            ...
            r.message
                    ?: "خطا در به‌روزرسانی بازیکن مسابقه"
        )
    } catch (e: Exception) {
        NetworkResult.Error(
            e.message

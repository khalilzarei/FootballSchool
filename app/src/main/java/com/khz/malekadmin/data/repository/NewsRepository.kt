...
import com.khz.malekadmin.data.dto.request.SetNewsAudiencesRequest
import com.khz.malekadmin.data.dto.request.UpdateNewsRequest
import com.khz.malekadmin.data.dto.response.NewsDto
import com.khz.malekadmin.data.remote.NewsApi
...
class NewsRepository(private val api: NewsApi) {
    ...
    ): NetworkResult<News> = try {
        val r = api.updateNews(
            id,
            request
        )
        val dto = r.data.unwrap<NewsDto>("news")
    ...
    suspend fun setAudiences(
        id: Int,
        request: SetNewsAudiencesRequest
    ): NetworkResult<Unit> = try {
        val r = api.setAudiences(
            id,
            request
        )
        if (r.success) NetworkResult.Success(Unit) else NetworkResult.Error(

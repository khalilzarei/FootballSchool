...
import com.khz.malekadmin.data.dto.request.CreateClassRequest
import com.khz.malekadmin.data.dto.request.CreateScheduleRequest
import com.khz.malekadmin.data.dto.request.EnrollPlayerRequest
import com.khz.malekadmin.data.dto.request.UpdateClassRequest
import com.khz.malekadmin.data.dto.request.UpdateEnrollmentRequest
import com.khz.malekadmin.data.dto.request.UpdateScheduleRequest
import com.khz.malekadmin.data.dto.response.ClassDto
import com.khz.malekadmin.data.dto.response.ClassScheduleDto
import com.khz.malekadmin.data.dto.response.EnrollAgeGroupResultDto
import com.khz.malekadmin.data.dto.response.EnrollmentDto
import com.khz.malekadmin.data.remote.ClassApi
import com.khz.malekadmin.data.remote.ClassScheduleApi
import com.khz.malekadmin.data.remote.EnrollmentApi
...
class ClassRepository(
    ...
    suspend fun createClass(
        ...
        val request = CreateClassRequest(
            title = title,
            seasonId = seasonId,
            ageGroupId = ageGroupIds?.firstOrNull()
                    ?: ageGroupId,
            ageGroupIds = ageGroupIds?.takeIf { it.isNotEmpty() },
            coachId = coachId,
            ...
            billingMonths = if (billingCycle == "seasonal") 3 else 1,
        ...
        Log.d(
            TAG,
            "createClass: $title cycle=$billingCycle monthly=$monthlyFee seasonal=$seasonalFee"
        )
    ...
    suspend fun updateClass(
        ...
        val request = UpdateClassRequest(
            title = title,
            seasonId = seasonId,
            // اگر لیست چندگانه ارسال شود، سرور اولویت را با آن می‌دهد
            ageGroupId = if (ageGroupIds != null) null else ageGroupId,
            ...
            monthlyFee = monthlyFee,
            seasonalFee = seasonalFee,
            billingMonths = if (billingCycle == "seasonal") 3 else if (billingCycle == "monthly") 1 else null,
            sessionFee = sessionFee,
            registrationFee = registrationFee,
            startDate = startDate,
            endDate = endDate
        ...
        Log.d(
            TAG,
            "updateClass: id=$id, title=$title"
        )
        val r = api.updateClass(
            id,
            request
        )
        val dto = r.data.unwrap<ClassDto>("class")
        ...
        if (r.success && dto != null) {
            ...
            NetworkResult.Error(
                r.message
                        ?: "خطا در به‌روزرسانی کلاس"
            )
        }
    } catch (e: Exception) {
        Log.e(
            TAG,
            "updateClass error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }
    ...
    suspend fun getSchedules(classId: Int): NetworkResult<List<ClassSchedule>> = try {
        ...
        if (r.success && r.data != null) {
            ...
            NetworkResult.Error(
                r.message
                        ?: "خطا در دریافت برنامه هفتگی"
            )
        }
    } catch (e: Exception) {
        Log.e(
            TAG,
            "getSchedules error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }
    ...
    suspend fun createSchedule(
        ...
        Log.e(
            TAG,
            "createSchedule error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }
    ...
    suspend fun updateSchedule(
        ...
        if (r.success && dto != null) NetworkResult.Success(dto.toDomain())
            ...
            r.message
                    ?: "خطا در به‌روزرسانی برنامه"
        )
    } catch (e: Exception) {
        Log.e(
            TAG,
            "updateSchedule error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }

    suspend fun toggleScheduleStatus(
        scheduleId: Int,
        activate: Boolean
    ): NetworkResult<Unit> = try {
    ...
    suspend fun getClassPlayers(
        ...
        val r = enrollmentApi.getClassPlayers(
            classId,
            page,
            perPage,
            query,
        ...
        if (r.success && r.data != null) {
            ...
            NetworkResult.Error(
                r.message
                        ?: "خطا در دریافت بازیکنان کلاس"
            )
        }
    } catch (e: Exception) {
        Log.e(
            TAG,
            "getClassPlayers error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }
    ...
    suspend fun enrollPlayer(
        ...
        val r = enrollmentApi.enrollPlayer(
            ...
            EnrollPlayerRequest(
                ...
                seasonalFeeOverride = seasonalFeeOverride,
        ...
        if (r.success && dto != null) NetworkResult.Success(dto.toDomain())
            ...
            r.message
                    ?: "خطا در ثبت‌نام بازیکن"
        )
    } catch (e: Exception) {
        Log.e(
            TAG,
            "enrollPlayer error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }

    suspend fun toggleEnrollmentStatus(
        enrollmentId: Int,
        activate: Boolean
    ): NetworkResult<Unit> = try {
    ...
    suspend fun enrollAgeGroup(classId: Int): NetworkResult<EnrollAgeGroupResultDto> = try {
        ...
        if (r.success && r.data != null) {
            ...
            NetworkResult.Error(
                r.message
                        ?: "خطا در ثبت‌نام گروهی"
            )
        }
    } catch (e: Exception) {
        Log.e(
            TAG,
            "enrollAgeGroup error: ${e.message}",
            e
        )
        NetworkResult.Error(ApiErrorHandler.extractMessage(e))
    }
